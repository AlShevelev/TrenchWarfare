...
class FromStartTurnInitialConfirmationOnStarTurnConfirmed {
  ...
  State process() {
    ...
    const isDebug = !Logger.turnedOff;

    // A human player turns first, so we can init a game field for the human only
    if (!_context.isAI) {
      for (final cell in _context.gameField.cells) {
        if (isDebug) {
          events.add(UpdateCellWithDebugInfo(cell));
        }

        if (cell.nation != null && // A cell has some data or we need to draw a border
            (!cell.isEmpty || _context.gameField.findCellsAround(cell).any((ca) => ca.nation != cell.nation))) {
